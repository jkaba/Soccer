...
espn_nwsl_game_all <- function(game_id){
  ...
  tryCatch(
    expr = {
      ...
      plays <- raw_play_df[["commentary"]] %>% tidyr::unnest_wider(.data$play.participants, names_sep = "_")
      suppressWarnings(
        aths <- plays %>%
    ...
    error = function(e){
      message(glue::glue("{Sys.time()}: Invalid arguments or no data for {game_id} available"))
    },
    warning = function(w){},
    finally = {}
  )
...
# Get NWSL Team Box Score Data from ESPN
#' @param game_id Game ID
...
# Get NWSL team names + ids from ESPN
#' @keywords NWSL Teams
...
# Get the NWSL schedule for a specific season/date via ESPN API
  ...
  tryCatch(
    expr = {
      ...
      if("broadcasts" %in% names(nwsl_data)){
        nwsl_data %>% tidyr::hoist(.data$broadcasts, broadcast_market = list(1, "market"), broadcast_name = list(1, "names", 1)) %>%
          dplyr::select(!where(is.list)) %>% janitor::clean_names()
      } 
      else{
        nwsl_data %>% dplyr::select(!where(is.list)) %>% janitor::clean_names()
      }
    },
    error = function(e) {
...
espn_nwsl_standings <- function(year){
  ...
  check_status(res)
  tryCatch(
    expr = { resp <- res %>% httr::content(as = "text", encoding = "UTF-8")
      raw_standings <- jsonlite::fromJSON(resp)[["children"]][["standings"]]
      
      # Create a dataframe of all NWSL teams by extracting from the raw_standings file
      ...
      # Use pivot_wider to transpose the dataframe so that we have a standings row for each team
