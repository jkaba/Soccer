# Get MLS Game Data from ESPN (pbp, team)
# Author: Jameel Kaba
...
espn_mls_game_all <- function(game_id){
  ...
  tryCatch(
    expr = {
      ...
      plays <- raw_play_df[["commentary"]]  %>% tidyr::unnest_wider(.data$play.participants)
      suppressWarnings(aths <- plays %>%
                         dplyr::groub_by(.data$play.id) %>%
      ...
      plays_df <- dplyr::bind_cols(plays, aths) %>% select(-.data$athlete.displayName)
      plays_df <- plays_df %>% janitor::clean_names()
    },
    
    error = function(e){
      message(glue::glue("{Sys.time()}: Invalid arguments or no play by play data for {game_id}"))
    },
  ...
  tryCatch(
    ...
    error = function(e) {
      message(glue::glue("[Sys.time()}: Invalid arguments or no team bos score for {game_id}"))
    },
...
# Get MLS play by play data from ESPN
...
# Get Team box score data from ESPN 

#' @param game_id Game ID
#' @return Returns a team boxscore data frame
#' @keywords MLS Team Box
#' @importFrom rlang .data
#' @importFrom jsonlite fromJSON toJSON
#' @importFrom dplyr filter select rename bind_cols bind_rows
#' @importFrom tidyr unnest unnest_wider everything
...
espn_mls_team_box <- function(game_id){
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  play_base_url <- "http://cdn.espn.com/soccer/commentary?render=false&userab=1&xhr=1&"
...
# Get MLS Team names and ID's from ESPN
...
espn_mls_teams <- function(){
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  play_base_url <- "http://site.api.espn.com/apis/site/v2/sports/soccer/usa.1/teams?limit=1000"
  ...
  leagues <- jsonlite::fromJSON(resp)[["sports"]][["leagues"]][[1]][['teams']][[1]][['team']] %>%
    ...
    dplyr::select(-.data$logos_width,-.data$logos_height, -.data$logos_alt, -.data$logos_rel) %>%
    dplyr::ungroup()
...
espn_mls_scoreboard <- function(season){
  ...
  season_dates <- season
  schedule_api <- glue::glue("http://site.api.espn.com/apis/site/v2/sports/soccer/usa.1/scoreboard?limit=1000&dates={season_dates}")
  ...
  tryCatch(
    expr = {
      ...
      mls_data <- raw_sched[["events"]] %>%
        ...
        tidyr::hoist(.data$status, status_name = list("type", "name")) %>%
      ...
      if("broadcasts" %in% names(mls_data)) {
        mls_data %>% tidyr::hoist(.data$broadcasts, broadcast_market = list(1, "market"), broadcast_name = list(1, "names", 1)) %>%
          dplyr::select(!where(is.list)) %>% janitor::clean_names()
      } 
      else {
        mls_data %>% dplyr::select(!where(is.list)) %>% janitor::clean_names()
      }
    },
    error = function(e) {
...
# GetcMLS Standings from ESPN

#' @param year Either numeric or character (YYYY)
#' @return Returns a tibble
#' @keywords MLS Standings
#' @importFrom rlang .data
#' @importFrom jsonlite fromJSON toJSON
...
#' }
espn_mls_standings <- function(year){
  
  standings_url <- "https://site.web.api.espn.com/apis/v2/sports/soccer/usa.1/standings?region=us&lang=en&contentorigin=espn&type=0&level=1&sort=winpercent%3Adesc%2Cwins%3Adesc%2Cgamesbehind%3Aasc&"
  ...
  tryCatch(
    expr = {
      ...
      teams <- teams %>% dplyr::select(.data$id, .data$displayName) %>%
        dplyr::rename(team_id = .data$id, team = .data$displayName)
      ...
      # Replace NA's in the dataframe with the correct corresponding values and removing all unnecessary columns
      ...
      standings_data <- standings_data %>% dplyr::select(.data$.id, .data$type, .data$value)
      
      # Use pivot_wider to transpose the dataframe so that we now have a standings row for each team
      standings_data <- standings_data %>% tidyr::pivot_wider(names_from = .data$type, values_from = .data$value)
      standings_data <- standings_data %>% dplyr::select(-.data$.id)
      
      # Join the 2 dataframes together to create a standings table
